package io.github.artiship.allo.worker.common;
...
public class CommonCache {
    ...
    private static ConcurrentHashMap<Long, TaskBo> CACHE_TASK = new ConcurrentHashMap<>();
    ...
    public static void cachePid(TaskBo task, Long pid) {
    ...
    public static Long getPid(TaskBo task) {
        synchronized (CACHE_PID) {
            return CACHE_PID.get(task.getId());
        }
    }

    public static void removePid(TaskBo taskBo) {
        synchronized (CACHE_PID) {
    ...
    public static void cacheApplicationId(TaskBo task, String applicationId) {
        synchronized (CACHE_APPLICATIONID) {
            CACHE_APPLICATIONID.put(task.getId(), applicationId);
        }
    }

    public static String getApplicationId(TaskBo task) {
        synchronized (CACHE_APPLICATIONID) {
    ...
    public static void removeApplicationId(TaskBo task) {
        synchronized (CACHE_APPLICATIONID) {
    ...
    public static List<TaskBo> cloneRunningTask() {
        synchronized (CACHE_PID) {
            List<TaskBo> tasks = Lists.newArrayListWithExpectedSize(CACHE_PID.size());
            CACHE_TASK.entrySet()
